...
for j = 1:2
    ...
    for i = 1:3
        ...
        handles(i, j) = axes('Units','normalized',...
                             'Position', [startX startY figSizeX figSizeY]); %#ok<LAXES>
        
        startX = startX + figSizeX + figGap - 0.06;
    end
    startY = startY - 0.5;
end
...
plotDataSet(handles(1, 1), rawData.Xtrn(:, rawData.I), rawData.ytrn,...
                                'Original (2d+8d garbage)');
                    
% Plot data projected onto space learned with MDML: hinge + frobenius                    
plotDataSet(handles(2, 1), Xptrn, rawData.ytrn, 'Projected Data (F + H)');
...
plotDataSet(handles(3, 1), Xptrn, rawData.ytrn, 'Projected Data (VN + L)');
